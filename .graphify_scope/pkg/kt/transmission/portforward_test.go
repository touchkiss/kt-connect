...
func Test_closeGone(t *testing.T) {
	t.Run("close once", func(t *testing.T) {
		...
		case <-time.After(time.Second):
			t.Fatal("expected gone channel to close")
		}
	})

	t.Run("ignore nil channel", func(t *testing.T) {
		closeGone(nil, &sync.Once{})
		closeGone(make(chan int), nil)
	})
}
