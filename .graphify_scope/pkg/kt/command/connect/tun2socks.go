...
func ByTun2Socks() error {
	if opt.Get().Connect.DisableTunDevice && opt.Get().Connect.Lane != "" {
		return fmt.Errorf("--lane requires tun device (do not use --disableTunDevice)")
	}
	...
	localSshPort, err := setupShadowPortForward(podName, common.StandardSshPort, "shadow SSH")
	if err != nil {
		return err
	}
