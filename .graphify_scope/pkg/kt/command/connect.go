...
func Connect() error {
	...
	opt.Store.Lane = opt.Get().Connect.Lane
...
func checkPermissionAndOptions() error {
	...
	if opt.Get().Connect.Lane != "" {
		if opt.Get().Connect.ShareShadow {
			return fmt.Errorf("--lane cannot be used with --shareShadow; lane-aware connect requires a dedicated shadow")
		}
		lane := opt.Get().Connect.Lane
		if len(lane) > 63 {
			return fmt.Errorf("invalid lane '%s': length must be <= 63", lane)
		}
		for i, ch := range lane {
			if (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9') || ch == '-' {
				// ok
			} else {
				return fmt.Errorf("invalid lane '%s': only [a-z0-9-] allowed", lane)
			}
			if (i == 0 || i == len(lane)-1) && !((ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9')) {
				return fmt.Errorf("invalid lane '%s': must start and end with [a-z0-9]", lane)
			}
		}
	}
	return nil
